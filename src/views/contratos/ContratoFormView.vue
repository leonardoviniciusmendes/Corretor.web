...
const backTo = computed(() => `/leads/${props.leadId}/contratos`)

async function submit() {
  const result = await action.run(async () => {
    await contratosService.create({}, props.leadId)
    return true
  }, 'Contrato criado.')
...
<template>
  ...
  <section class="panel form-panel">
    <form @submit.prevent="submit">
      ...
      <div class="form-actions"><RouterLink class="button secondary" :to="backTo">Cancelar</RouterLink><button class="button" type="submit" :disabled="action.loading.value">Criar contrato</button></div>
    </form>
  </section>
</template>
